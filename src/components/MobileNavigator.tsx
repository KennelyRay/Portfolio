...
export function MobileNavigator() {
  ...
  return (
    <motion.div
      ...
      className="fixed inset-x-0 bottom-4 z-40 px-4 xl:hidden"
    >
      <div className="mx-auto max-w-xl rounded-[1.8rem] border border-white/10 bg-[#05070a]/85 px-2 py-2 shadow-[0_0_40px_rgba(0,168,255,0.08)] backdrop-blur-xl">
          ...
          {navItems.map((item) => {
            ...
            return (
              <a
                key={item.id}
                href={item.href}
                className={`flex min-h-14 flex-col items-center justify-center rounded-[1.2rem] px-2 py-2 text-center transition-colors duration-300 ${
                  isActive
                    ? "bg-[var(--color-brand-blue)]/12 text-white"
